                        ...
                        <div class="table_section padding_infor_info">
                           <div class="table-responsive-sm">
                              <form method="post">
                                 ...
                                 <button type="submit" name="delete" class="main_bt">delete</button>
                              </form>
                           </div>
                        </div>
                     </div>
...
if (isset($_POST['delete'])) {
   $num = $_POST['chech'];
   $idMark = getFromData("SELECT mark.id_mark FROM mark WHERE mark.id_student='$num';");
   foreach ($idMark as $result) {
      $id = $result["id_mark"];
      connToData("DELETE FROM mark WHERE mark.id_mark='$id';");
   }
   connToData("DELETE FROM student WHERE student.id_student='$num';");

   // $data3 = getFromData("SELECT account.id_account FROM account JOIN student
   // ON student.id_account =account.id_account
   // WHERE student.id_student='$num'
   //     ORDER BY account.id_account DESC LIMIT 1
   //  ");
   // foreach ($data3 as $res) {
   //    $idAccount = $res["id_account"];
   // }
   // connToData("DELETE FROM account WHERE account.id_account='$idAccount'");
}
?>